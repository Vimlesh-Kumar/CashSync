...
function decodeJwt(token: string) {
    ...
    if (parts.length !== 3) {
        throw { status: 400, message: 'Invalid OAuth token format.' };
    }
...
function validateAud(aud: string | string[] | undefined, allowed: string[]) {
    ...
    if (!ok) {
        throw { status: 401, message: 'OAuth token audience does not match this app.' };
    }
}
...
async function getAppleKeys(): Promise<AppleJwk[]> {
    ...
    if (!res.ok) {
        throw { status: 502, message: 'Failed to fetch Apple signing keys.' };
    }
...
async function verifyAppleIdToken(idToken: string): Promise<VerifiedOAuthIdentity> {
    ...
    if (payload.iss !== 'https://appleid.apple.com') {
        throw { status: 401, message: 'Invalid Apple token issuer.' };
    }

    const nowSec = Math.floor(Date.now() / 1000);
    if (!payload.exp || payload.exp < nowSec) {
        throw { status: 401, message: 'Apple token is expired.' };
    }
    ...
    if (!jwk) {
        throw { status: 401, message: 'Unable to match Apple signing key.' };
    }
    ...
    if (!valid) {
        throw { status: 401, message: 'Apple token signature verification failed.' };
    }
...
async function verifyGoogleIdToken(idToken: string): Promise<VerifiedOAuthIdentity> {
    ...
    if (!res.ok || payload.error_description) {
        throw { status: 401, message: payload.error_description || 'Invalid Google token.' };
    }

    const nowSec = Math.floor(Date.now() / 1000);
    const exp = Number(payload.exp ?? '0');
    if (!exp || exp < nowSec) {
        throw { status: 401, message: 'Google token is expired.' };
    }
